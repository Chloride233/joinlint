...
HARNESS_PROMPT = """The schema is already provided, so your first tool call must be JoinLint get_join_plan. Call get_join_plan once after choosing every intended physical table instance. Each entity_refs item must be exactly {"ref":"orders","entity":"orders"}: ref is a unique request-local alias and entity is the physical table name; repeat an entity only for a genuine self join. Set expected_grain_ref to the instance whose unique key must remain one row per output row before aggregation. For a many-to-one join, the referencing child normally preserves that grain; aggregation, DISTINCT, and GROUP BY do not restore grain in Stage 1. If planning returns UNCONNECTED_ENTITY_REF, you may make exactly one changed replan that removes or replaces only the affected references, but only after confirming from the task and schema that they are not needed; otherwise submit empty SQL with the stable code in warning. If planning returns GRAIN_INCOMPATIBLE, COMPOUND_FANOUT, NO_VERIFIED_PATH, or any other non-ok status, do not retry variants or guess: submit empty SQL with the stable code in warning. Use only returned proof predicates. Call JoinLint validate_sql with the exact final SQL and returned plan_id, then call execute_sql only after validation passes. If validation is not ok, do not execute and submit empty SQL with the stable code. JoinLint proof is not query correctness."""
HARNESS_PROMPT += " Call JoinLint validate_sql exactly once; never repeat a JoinLint call after a result. Call execute_sql at most once after validation, then submit immediately without revising or rerunning the SQL."
