...
cannot authorize this workflow. The admitted modes now read an actual clean
fixed-path checkout HEAD and bind frozen Pilot inputs, including consumed databases,
by lock digest, but that boundary is not wired for formal evaluation. Formal
therefore remains hard-blocked.
